...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Jika email confirmation NYALA di Supabase, session akan null
      if (data.session === null && data.user) {
        setSuccessMessage(
          "Pendaftaran berhasil! Silakan cek kotak masuk atau folder spam email Anda untuk mengaktifkan akun."
        );
        // Jangan langsung redirect, biarkan user baca pesan
      } else {
        // Jika email confirmation MATI, langsung login & ke home
        router.push("/");
        setTimeout(() => {
          router.refresh();
        }, 500);
  ...
  return (
    <div className="min-h-screen bg-emerald-50 dark:bg-gray-900 flex items-center justify-center p-4 transition-colors duration-300">
      <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-[2rem] shadow-2xl shadow-emerald-500/10 p-6 md:p-8 border border-white dark:border-gray-700">
        ...
        {successMessage && (
          <div className="mb-4 p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl text-emerald-700 dark:text-emerald-400 text-sm font-medium text-center">
            {successMessage}
          </div>
        )}
        ...
        <div className="relative my-6 text-center">
          ...
          <span className="relative px-4 bg-white dark:bg-gray-800 text-xs text-gray-500">Atau daftar dengan</span>
        </div>
